...
describe('AdminInventory Component', () => {
  ...
  test('shows confirmation dialog when Delete clicked', () => {
    ...
    expect(global.confirm).toHaveBeenCalledWith('Are you sure you want to delete this vehicle?');
  });
  ...
  test('shows loading state during delete operation', async () => {
    global.confirm = jest.fn(() => true);
    mockDeleteVehicle.mockImplementation(() => new Promise(resolve => setTimeout(resolve, 100)));
